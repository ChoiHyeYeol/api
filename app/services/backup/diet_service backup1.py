...
class DietPlanner:
    ...
    def generate_weekly_plan(self, user_type: str, preferences: List[str], allergies: List[str]) -> Dict[int, List[List[str]]]:
        ...
        for day in range(7):
            ...
            for i, slot in enumerate(MEAL_SLOTS):
                ...
                # 확률적으로 뽑되, 상위 N 내에서만 샘플링
                if i == 0:
                    r_pick = self._sample_bundle(r_ranked, self.rec_mem, count=2, topn=6, temperature=0.7)
                    f_pick = self._sample_bundle(f_ranked, self.food_mem, count=1, topn=6, temperature=0.7)
                elif i == 1:
                    f_pick = self._sample_bundle(f_ranked, self.food_mem, count=1, topn=6, temperature=0.7)
                    r_pick = self._sample_bundle(r_ranked, self.rec_mem, count=2, topn=6, temperature=0.7)
                elif i == 2:
                ...
                meal = r_pick + f_pick if i % 2 == 0 else f_pick + r_pick

                # 안전장치
                if not meal:
                    meal = (r_candidates[:1] or f_candidates[:1] or [])
