...
def select_foodcodes(index: Dict[str, FoodRow], constraints: Dict[str, Any],
    ...
    for code, row in index.items():
        ...
        if constraints.get("prefer_low_gi", True) and row.gi is not None and row.gi > 55:
            continue
        # 100g 기준이라 느슨히 허용
        if not (lo * 0.5 <= _nz(row.carb_g) <= hi * 1.5):
            continue
        if _nz(row.kcal) > kcal_max:
            continue
        if _nz(row.sugar_g) > constraints.get("max_added_sugar_g", 10) + 5:
            continue
        if _nz(row.sodium_mg) > sodium_max:
            continue
        pref_boost = 1 if any(p.lower() in row.name.lower() for p in prefs) else 0
        score = (1000 - (_nz(row.gi) if _finite(row.gi) else 50)) + pref_boost*10 + max(0, 10 - abs(((lo+hi)/2) - _nz(row.carb_g)))
