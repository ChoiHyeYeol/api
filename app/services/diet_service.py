...
class DataPaths:
    ...
    food_kor_path: Optional[str] = "app/data/diet/filtered_gdm_foods.xlsx"      # "음식 영양성분" (csv/xlsx) -> must have columns: 식품코드, 식품명, 에너지(kcal), 탄수화물(g), 단백질(g), 지방(g), 당류(g), 나트륨(mg), 식이섬유(g)
...
def select_foodcodes(index: Dict[str, FoodRow], constraints: Dict[str, Any],
    ...
    for code, row in index.items():
        ...
        kcal = _as_inf_if_missing(row.kcal)
        sugar = _as_inf_if_missing(row.sugar_g)
        sodium = _as_inf_if_missing(row.sodium_mg)
...
def _rule_check_meal(total: dict, constraints: dict, meal_slot: str) -> Tuple[bool, Dict[str, Any]]:
    ...
    chk("sugar_g", total["sugar_g"] <= constraints.get("max_added_sugar_g", 10), f"당류 {total['sugar_g']:.1f} > max")

    if constraints.get("prefer_low_gi", True):
        giw = total.get("gi_weighted")
        if _finite(giw):
            flag("gi", giw <= 55, f"가중 GI {giw:.0f} > 55")
        else:
