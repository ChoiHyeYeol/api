...
class HybridService:
    ...
    def __init__(self):
        ...
        try:
            ...
            txts, metas = [], []
            for name, pdf in [("GI-KR", PATHS["pdf_gi_kr"]), ("GI-INTL", PATHS["pdf_gi_intl"])
                              # , ("GIUDE1", PATHS["pdf_guide1"])
                              # , ("GIUDE2", PATHS["pdf_guide2"])
                              # , ("GIUDE3", PATHS["pdf_guide3"])
                              # , ("NU-BAL", PATHS["pdf_bal"])
                              ]:
                # print("raw : ")
                raw = _pdf_text(pdf)
                if not raw: continue
                splitter = RecursiveCharacterTextSplitter(chunk_size=900, chunk_overlap=150)
                for ch in splitter.split_text(re.sub(r"\s+", " ", raw).strip()):
                    txts.append(ch); metas.append({"source": name})
            if txts:
                self.vs = FAISS.from_texts(txts, embed, metadatas=metas)
